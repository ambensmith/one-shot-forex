...
- Fraunces serif for headings — editorial authority, anti-fintech personality
- Outfit for all functional text — clean, geometric, readable
...
### Font Families
- **Serif (Display)**: `'Fraunces', Georgia, 'Times New Roman', serif` — headings, narrative titles, pull quotes, trade story chapter headings. Loaded via Google Fonts at weights 400, 500, 600, 700.
- **Sans-Serif (Functional)**: `'Outfit', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif` — body text, labels, navigation, tables, buttons, data.
...
|------|------|------|--------|-------------|----------------|-------|
| Page Title | Fraunces | 36px (2.25rem) | 700 | 1.15 | -0.5px | Top-level page headings: "Dashboard", "LLM Analysis" |
| Section Heading | Fraunces | 28px (1.75rem) | 600 | 1.20 | -0.3px | Major sections: "Open Trades", "Pipeline Activity" |
| Card Heading | Fraunces | 22px (1.375rem) | 600 | 1.25 | -0.2px | Card titles, trade pair names in drill-down |
| Chapter Title | Fraunces | 18px (1.125rem) | 500 | 1.30 | normal | Timeline chapter headings in narrative drill-down |
| Body Large | Outfit | 16px (1rem) | 400 | 1.65 | normal | Primary reading text, narrative content, descriptions |
| Body | Outfit | 14px (0.875rem) | 400 | 1.60 | normal | Standard body text, table cells, form content |
| Label | Outfit | 14px (0.875rem) | 500 | 1.40 | normal | Form labels, table headers, nav items |
| Button | Outfit | 14px (0.875rem) | 500 | 1.00 | 0.1px | Button text — slightly tracked for clarity |
| Caption | Outfit | 12px (0.75rem) | 400 | 1.50 | normal | Timestamps, metadata, secondary info |
| Badge | Outfit | 11px (0.6875rem) | 600 | 1.00 | 0.3px | Status badges, tags, small labels |
| Overline | Outfit | 11px (0.6875rem) | 600 | 1.00 | 0.8px | Uppercase category labels above sections |
...
| Data | Outfit | 14px (0.875rem) | 500 | 1.40 | normal | Numeric data in tables — uses `font-variant-numeric: tabular-nums` |
| Data Large | Outfit | 24px (1.5rem) | 600 | 1.20 | -0.3px | Hero metrics — account value, total P&L |

### Principles
- **Serif for personality, sans-serif for function.** Fraunces appears only in headings and narrative contexts. It never appears in tables, buttons, navigation, or data displays. The moment you see a serif, you know you're reading something editorial — a title, a story heading, a pull quote.
...
- **Restrained weight range.** Outfit uses 400 (body), 500 (labels/buttons), 600 (badges/data emphasis). Fraunces uses 500–700. No ultra-bold anywhere — the design speaks through space and shadow, not weight.
...
- Padding: `10px 20px`
- Radius: `8px`
- Font: Outfit 14px weight 500, letter-spacing 0.1px
...
- Padding: `2px 8px`
- Radius: `6px`
- Font: Outfit 11px weight 600, letter-spacing 0.3px
...
- Padding: `2px 8px`
- Radius: `6px`
- Font: Outfit 11px weight 600, letter-spacing 0.5px, uppercase

### Tables
Tables are the workhorse of the dashboard. They should feel open and readable.

- Header: Outfit 12px weight 600, uppercase, letter-spacing 0.5px, color `#A8A29E`, border-bottom `1px solid #E7E5E4`
...
- Label: Outfit 14px weight 500, color `#1C1917`, margin-bottom `6px`
...
**Panel Content:**
- Left side: Account total — Outfit 18px weight 600, color `#FAFAF9`, `tabular-nums`
- Right side: Overall P&L percentage — Outfit 14px weight 500, colored with semantic text color (green if positive, red if negative)
...
**Chapter Heading:**
- Font: Fraunces 18px weight 500
- Color: `#1C1917` (completed) or `#A8A29E` (pending)
- Margin-bottom: `8px`

**Chapter Body (completed):**
- Font: Outfit 14px weight 400, line-height 1.60
- Color: `#57534E`
...
- Font: Outfit 14px weight 400, italic
- Color: `#A8A29E`
...
- Radius: `2px`
- Label: "82% confidence" in Outfit 12px weight 500, semantic text color
...
┌──────────────────────────────────────────────────────────────┐
│  [Fraunces] Dashboard                                        │
│  [Outfit caption] Last updated 2 minutes ago                  │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐          │
...
- Each card shows: instrument name (Outfit 12px weight 600, uppercase), direction arrow (▲/▼/—), bias label ("Bullish"/"Bearish"/"Neutral" in Outfit 11px), strength as a thin bar (same as confidence bar: 4px height, semantic color fill)
...
- Empty state: "No open trades. The system is monitoring for signals." in Outfit 14px italic, `#A8A29E`
...
```
┌──────────────────────────────────────────────────────────────┐
│  [Fraunces] LLM Analysis                                     │
│  [Outfit caption] Pipeline activity from the last 12 hours    │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  [Fraunces] Recent Headlines                                 │
...
│                                                              │
├──────────────────────────────────────────────────────────────┤
│  [Fraunces] Relevance Assessments                            │
│                                                              │
│  ┌──────────────────────────────────────────────────────┐   │
...
├──────────────────────────────────────────────────────────────┤
│  [Fraunces] Prompts                                          │
│  [Outfit caption] Edit the prompts used in each pipeline stage│
│                                                              │
│  ┌─ Relevance Prompt (v3) ──────────────────────────────┐   │
...
- Each group: source name as overline label (Outfit 11px weight 600, uppercase, `#A8A29E`) with count badge
- Headlines as a compact list: headline text in Outfit 14px weight 400, truncated to 2 lines, timestamp in Outfit 12px caption below
...
- Each item: headline text (Outfit 14px weight 500), arrow (→), instrument tags (Instrument Tag styling), reasoning below in Outfit 14px weight 400 `#57534E`
...
- Content: instrument as Card Heading (Fraunces 22px), direction + confidence bar, reasoning as prose (Outfit 14px, max-width 720px), key factors as a subtle inline list, challenge result as a separate sub-section with lighter background (`#F5F5F4`)
...
- Section header: prompt name in Fraunces 18px weight 500 + version badge (Outfit 11px)
...
┌──────────────────────────────────────────────────────────────┐
│  [Fraunces] Strategies                                       │
│  [Outfit caption] Traditional quantitative trading strategies  │
├──────────────────────────────────────────────────────────────┤
│                                                              │
│  ┌─ Momentum ────────────────────────────────────────────┐  │
│  │  [Fraunces heading]  Momentum                         │  │
│  │  [Outfit body] Measures the tendency of assets that     │  │
│  │  have been rising to continue rising...                │  │
│  │                                                       │  │
│  │  [Overline] HOW IT WORKS                              │  │
│  │  [Outfit body] Compares 12-month return against         │  │
...
- Card heading: Fraunces 22px weight 600
- Strategy description: Outfit 16px weight 400, line-height 1.65, `#57534E` — written for someone reading the university report, not a developer. Should explain what the strategy does and why it might work. Max-width `720px`.
- "How it works" section: separated by overline label, Outfit 14px, explains the calculation in plain English
- Parameters: displayed as a row of key-value pairs in a subtle inline layout (Outfit 12px weight 500 label, Outfit 14px weight 400 value)
...
Standard card layout, no semantic shadows. Outfit throughout — no Fraunces needed on this page (it's purely functional).

## 10. Do's and Don'ts

### Do
- Use Fraunces **only** for headings and narrative chapter titles — it should feel special, not ubiquitous
- Use Outfit for all functional UI — tables, buttons, nav, labels, form inputs, data
...
- Don't use Fraunces in tables, buttons, navigation, or form inputs — it's reserved for editorial moments
...
| Role | Desktop | Mobile |
|------|---------|--------|
| Page Title | 36px Fraunces | 28px Fraunces |
| Section Heading | 28px Fraunces | 22px Fraunces |
| Card Heading | 22px Fraunces | 18px Fraunces |
| Body Large | 16px Outfit | 16px Outfit (unchanged) |
| Body | 14px Outfit | 14px Outfit (unchanged) |
...
- Page Title: Fraunces 36px / 700 / -0.5px
- Section Heading: Fraunces 28px / 600 / -0.3px
- Card Heading: Fraunces 22px / 600 / -0.2px
- Chapter Title: Fraunces 18px / 500
- Body Large: Outfit 16px / 400 / line-height 1.65
- Body: Outfit 14px / 400 / line-height 1.60
- Label: Outfit 14px / 500
- Button: Outfit 14px / 500 / letter-spacing 0.1px
- Caption: Outfit 12px / 400
- Badge: Outfit 11px / 600 / letter-spacing 0.3px
- Data: Outfit 14px / 500 / tabular-nums
- Data Large: Outfit 24px / 600 / -0.3px
...
**Trade Card:**
"Create a card with white background, 1px solid rgba(34,197,94,0.20) border, 10px radius. Shadow: 0 4px 24px rgba(34,197,94,0.15), 0 1px 3px rgba(0,0,0,0.04). Background tint: rgba(34,197,94,0.04). Title in Fraunces 22px weight 600, color #1C1917. Body in Outfit 14px weight 400, color #57534E. Status badge: rgba(34,197,94,0.08) background, #15803D text, Outfit 11px weight 600, 6px radius."

**Page Header:**
"Create a page header with Fraunces 36px weight 700, color #1C1917, letter-spacing -0.5px. Subtitle in Outfit 16px weight 400, color #57534E, line-height 1.65. Below, an overline label in Outfit 11px weight 600, color #A8A29E, letter-spacing 0.8px, uppercase."

**Bottom Pill Nav:**
"Create a fixed bottom navigation bar: background rgba(28,25,23,0.92), backdrop-filter blur(12px), border-radius 16px, padding 6px 8px, centered at bottom 20px. Nav items in Outfit 13px weight 500. Active item has rgba(250,250,249,0.12) pill background with 10px radius and white text. Inactive items at 50% opacity. Chevron-up icon on the right."

**Timeline Chapter (Completed):**
"Vertical timeline node: 12px circle, white fill, 2px solid #D6D3D1 border. Chapter heading in Fraunces 18px weight 500, color #1C1917. Body in Outfit 14px weight 400, color #57534E, line-height 1.60, max-width 720px."

**Timeline Chapter (Pending):**
"Vertical timeline node: 12px circle, transparent fill, 2px dashed #D6D3D1 border. Chapter heading in Fraunces 18px weight 500, color #A8A29E. Body in Outfit 14px italic weight 400, color #A8A29E: 'Awaiting signal generation...'"

### Iteration Guide
1. Always check: is this a heading/narrative moment (→ Fraunces) or a functional/data moment (→ Outfit)?
